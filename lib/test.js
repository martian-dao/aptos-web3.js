...
describe("integration test", function() {
    it("should be able to create new wallet and airdrop", async function() {
        const bob = await apis.create_wallet();
        await apis.air_drop(bob['code'], 420);
        assert.strictEqual(await apis.get_balance(bob["address key"]), 430); // create_wallet() airdrops 10 coins
    });

    it("should be able to import wallet", async function() {
        const bob = await apis.create_wallet();
        await apis.air_drop(bob['code'], 420);
        const bob2 = await apis.import_wallet(bob['code'])
        assert.strictEqual(await apis.get_balance(bob2["address key"]), 440); // create_wallet() and import_wallet airdrops 10 coins
    });

    it("should be able to transfer", async function() {
        const alice = await apis.create_wallet();
        await apis.air_drop(alice['code'], 5000);
        const bob = await apis.create_wallet();

        await apis.transfer(alice["code"], bob["address key"], 20);

        assert.strictEqual(await apis.get_balance(bob["address key"]), 30); // create_wallet() airdrops 10 coins
    });
    ...
    it("should be able to transfer NFT", async function() {
        const alice = await apis.create_wallet();
        await apis.air_drop(alice['code'], 10000);

        const bob = await apis.create_wallet();
        await apis.air_drop(bob['code'], 10000);
