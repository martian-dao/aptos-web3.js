...
test("full tutorial faucet flow", async () => {
    ...
    const events = await client.getEventsByEventHandle(tx.sender, "0x1::coin::CoinStore<0x1::test_coin::TestCoin>", "withdraw_events");
    expect(events[0].type).toBe("0x1::coin::WithdrawEvent");
    const eventSubset = await client.getEventsByEventHandle(tx.sender, "0x1::coin::CoinStore<0x1::test_coin::TestCoin>", "withdraw_events", { start: 0, limit: 1 });
