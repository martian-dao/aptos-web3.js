...
describe("ABI", () => {
    it("parses create_acount successfully", async () => {
        const deserializer = new bcs_1.Deserializer(new hex_string_1.HexString(SCRIPT_FUNCTION_ABI).toUint8Array());
        ...
        expect(hex_string_1.HexString.fromUint8Array(moduleAddress.address).toShortString()).toBe("0x1");
        ...
        expect(arg.type_tag instanceof type_tag_1.TypeTagAddress).toBeTruthy();
    });
    it("parses script abi successfully", async () => {
        const deserializer = new bcs_1.Deserializer(new hex_string_1.HexString(TRANSACTION_SCRIPT_ABI).toUint8Array());
        ...
        expect(hex_string_1.HexString.fromUint8Array(transactionScriptABI.code).hex()).toBe(
        // eslint-disable-next-line max-len
        ...
        expect(arg.type_tag instanceof type_tag_1.TypeTagU64).toBeTruthy();
    });
});
