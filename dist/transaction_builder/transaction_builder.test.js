...
test("serialize entry function payload with no type args", () => {
    ...
    const rawTxn = new aptos_types_1.RawTransaction(aptos_types_1.AccountAddress.fromHex(new hex_string_1.HexString(ADDRESS_3)), 0n, entryFunctionPayload, 2000n, 0n, BigInt(TXN_EXPIRE), new aptos_types_1.ChainId(4));
...
test("serialize entry function payload with type args", () => {
    ...
    const rawTxn = new aptos_types_1.RawTransaction(aptos_types_1.AccountAddress.fromHex(ADDRESS_3), 0n, entryFunctionPayload, 2000n, 0n, BigInt(TXN_EXPIRE), new aptos_types_1.ChainId(4));
    const signedTxn = sign(rawTxn);
...
test("serialize module payload", () => {
    ...
    const signedTxn = sign(rawTxn);
    expect(hexSignedTxn(signedTxn)).toBe("");
});
