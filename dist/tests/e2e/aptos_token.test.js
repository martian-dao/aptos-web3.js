...
describe("token objects", () => {
    ...
    test("transfer non fungible token", async () => {
        ...
        await provider.waitForTransaction(await aptosToken.transfer({ owner: bob, tokenAddress, recipient: alice.address() }), {
            checkSuccess: true,
        });
        getTokenDataSpy.mockRestore();
    }, test_helper_test_1.longTestTimeout);
    test("transfer non fungible token when isFungibleToken param set to false", async () => {
        await provider.waitForTransaction(await aptosToken.transfer({ owner: alice, tokenAddress, recipient: bob.address() }, false), {
            checkSuccess: true,
        });
    }, test_helper_test_1.longTestTimeout);
    test("getTokenData indexer query is not being called when isFungibleToken param is set", async () => {
        const getTokenDataSpy = jest.spyOn(provider, "getTokenData");
