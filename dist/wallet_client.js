...
const transaction_1 = require("./transaction_builder/aptos_types/transaction");
...
class WalletClient {
    ...
    async importWallet(code) {
        ...
        for (let i = 0; i < MAX_ACCOUNTS; i += 1) {
            ...
            for (let j = 0; j < ADDRESS_GAP; j += 1) {
                ...
                const account = aptos_account_1.AptosAccount.fromDerivePath(derivationPath, code);
                if (j === 0) {
                    address = hex_string_1.HexString.ensure(account.address()).toShortString();
    ...
    async createNewAccount(code) {
        for (let i = 0; i < MAX_ACCOUNTS; i += 1) {
            ...
            const account = aptos_account_1.AptosAccount.fromDerivePath(derivationPath, code);
            const address = hex_string_1.HexString.ensure(account.address()).toShortString();
            const response = await (0, cross_fetch_1.default)(`${this.aptosClient.nodeUrl}/accounts/${address}`, {
                method: "GET",
            });
            if (response.status === 404) {
    ...
    static getTransactionDeserialized(bytes) {
        const deserializer = new bcs_1.Deserializer(bytes);
        return transaction_1.RawTransaction.deserialize(deserializer);
    }
    /**
    ...
    static getAccountFromMnemonic(code) {
        return aptos_account_1.AptosAccount.fromDerivePath(`m/44'/${COIN_TYPE}/0'/0'/0'`, code);
    }
    /**
    ...
    static getAccountFromMetaData(code, metaData) {
        return aptos_account_1.AptosAccount.fromDerivePath(metaData.derivationPath, code, metaData.address);
    }
    /**
    ...
    async transfer(account, recipient_address, amount) {
        try {
            ...
            const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
                transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(hex_string_1.HexString.ensure(recipient_address).toString())),
     ...
     * @returns transaction hash
     */
    async createToken(account, collection_name, name, description, supply, uri, max = consts_1.MAX_U64_BIG_INT, royalty_payee_address = account.address(), royalty_points_denominator = 0, royalty_points_numerator = 0, property_keys = [], property_values = [], property_types = []) {
        return Promise.resolve(await this.tokenClient.createToken(account, collection_name, name, description, supply, uri, max, royalty_payee_address, royalty_points_denominator, royalty_points_numerator, property_keys, property_values, property_types));
    }
    /**
    ...
    dst_address, amount) {
        ...
        const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::managed_coin", "mint", [token], [
            transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(hex_string_1.HexString.ensure(dst_address).toString())),
            transaction_builder_1.BCS.bcsSerializeUint64(amount),
        ]));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_client_1.AptosClient.generateBCSTransaction(account, rawTxn);
    ...
    to_address, amount) {
        const token = new transaction_builder_1.TxnBuilderTypes.TypeTagStruct(transaction_builder_1.TxnBuilderTypes.StructTag.fromString(coin_type_path));
        const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
            transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(hex_string_1.HexString.ensure(to_address).toString())),
            transaction_builder_1.BCS.bcsSerializeUint64(amount),
        ]));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_client_1.AptosClient.generateBCSTransaction(account, rawTxn);
    ...
    async publishModule(account, moduleHex) {
        const moduleBundlePayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadModuleBundle(new transaction_builder_1.TxnBuilderTypes.ModuleBundle([
            new transaction_builder_1.TxnBuilderTypes.Module(new hex_string_1.HexString(moduleHex).toUint8Array()),
        ]));
        const [{ sequence_number: sequenceNumber }, chainId] = await Promise.all([
            this.aptosClient.getAccount(account.address()),
            this.aptosClient.getChainId(),
        ]);
        const rawTxn = new transaction_builder_1.TxnBuilderTypes.RawTransaction(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account.address()), BigInt(sequenceNumber), moduleBundlePayload, 4000n, 1n, BigInt(Math.floor(Date.now() / 1000) + 10), new transaction_builder_1.TxnBuilderTypes.ChainId(chainId));
        const bcsTxn = aptos_client_1.AptosClient.generateBCSTransaction(account, rawTxn);
        const transactionRes = await this.aptosClient.submitSignedBCSTransaction(bcsTxn);
