...
class WalletClient {
    ...
    async importWallet(code) {
        ...
        for (let i = 0; i < MAX_ACCOUNTS; i += 1) {
            ...
            accountMetaData.push({
                derivationPath,
                address: hex_string_1.HexString.ensure(address).toShortString(),
                publicKey,
            });
        }
    ...
    async registerCoin(account, coin_type_path) {
        ...
        const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coins", "register", [token], []));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_client_1.AptosClient.generateBCSTransaction(account, rawTxn);
        const transactionRes = await this.aptosClient.submitSignedBCSTransaction(bcsTxn);
        await this.aptosClient.waitForTransaction(transactionRes.hash);
