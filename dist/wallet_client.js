...
class WalletClient {
    ...
    async createToken(account, collection_name, name, description, supply, uri, royalty_points_per_million = 0) {
        return Promise.resolve(await this.tokenClient.createToken(account, collection_name, name, description, supply, uri, royalty_points_per_million));
    }
    /**
    ...
    async offerToken(account, receiver_address, creator_address, collection_name, token_name, amount) {
        return Promise.resolve(await this.tokenClient.offerToken(account, receiver_address, creator_address, collection_name, token_name, amount));
    }
    /**
    ...
    async cancelTokenOffer(account, receiver_address, creator_address, collection_name, token_name) {
        return Promise.resolve(await this.tokenClient.cancelTokenOffer(account, receiver_address, creator_address, collection_name, token_name));
    }
    /**
     * claims offered NFT
     *
    ...
    async claimToken(account, sender_address, creator_address, collection_name, token_name) {
        return Promise.resolve(await this.tokenClient.claimToken(account, sender_address, creator_address, collection_name, token_name));
    }
    /**
    ...
    async getTokenIds(address, limit, start) {
        ...
        const countWithdraw = {};
        const tokenIds = [];
        const depositEvents = await this.getEventStream(address, "0x1::token::TokenStore", "deposit_events", limit, start);
        const withdrawEvents = await this.getEventStream(address, "0x1::token::TokenStore", "withdraw_events", limit, start);
    ...
    async getTokens(address, limit, start) {
        ...
        await Promise.all(tokenIds.map(async (tokenId) => {
            let resources;
            if (cache_1.default.has(`resources--${tokenId.data.creator}`)) {
                resources = cache_1.default.get(`resources--${tokenId.data.creator}`);
            }
            else {
                resources = await this.aptosClient.getAccountResources(tokenId.data.creator);
                cache_1.default.set(`resources--${tokenId.data.creator}`, resources);
            }
