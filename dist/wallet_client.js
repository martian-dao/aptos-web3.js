...
class WalletClient {
    ...
    async importWallet(code) {
        ...
        for (let i = 0; i < MAX_ACCOUNTS; i += 1) {
            ...
            for (let j = 0; j < ADDRESS_GAP; j += 1) {
                ...
                if (j === 0) {
                    address = hex_string_1.HexString.ensure(account.address()).toString();
    ...
    createNewAccount(code, index = 0) {
        ...
        const account = aptos_account_1.AptosAccount.fromDerivePath(derivationPath, code);
        const address = hex_string_1.HexString.ensure(account.address()).toString();
        return {
            derivationPath,
            address,
            publicKey: account.pubKey().toString(),
    ...
    async estimateGasFees(accountPublicKey, transaction) {
        const simulateResponse = await this.aptosClient.simulateTransaction(hex_string_1.HexString.ensure(accountPublicKey), transaction);
        ...
        const change = txnData.changes.filter((ch) => {
            ...
            if (write.data.type ===
                ...
                (write.address === accountAddress.toString() ||
                    write.address === hex_string_1.HexString.ensure(accountAddress).toShortString())) {
                return true;
            }
            return false;
        });
    ...
    static generateBCSSimulation(account, rawTxn) {
        return Promise.resolve(aptos_client_1.AptosClient.generateBCSSimulation(account.pubKey(), rawTxn));
    }
    ...
    dst_address, amount) {
        ...
        const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::managed_coin", "mint", [token], [
            BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(hex_string_1.HexString.ensure(dst_address).toString())),
    ...
    to_address, amount) {
        ...
        const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
            BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(hex_string_1.HexString.ensure(to_address).toString())),
            BCS.bcsSerializeUint64(amount),
        ]));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_client_1.AptosClient.generateBCSTransaction(account, rawTxn);
    ...
    async publishModule(sender, packageMetadataHex, moduleHex, extraArgs) {
        const txnHash = await this.aptosClient.publishPackage(sender, new hex_string_1.HexString(packageMetadataHex).toUint8Array(), [new transaction_builder_1.TxnBuilderTypes.Module(new hex_string_1.HexString(moduleHex).toUint8Array())], extraArgs);
        return txnHash;
    }
}
