...
test("generates random accounts", () => {
    ...
    const a2 = new aptos_account_1.AptosAccount();
...
test("generates derive path accounts", () => {
    ...
    const a1 = aptos_account_1.AptosAccount.fromDerivePath("m/44'/637'/0'/0'/0'", mnemonic);
    expect(a1.address().hex()).toBe(address);
});
test("generates derive path accounts", () => {
    expect(() => {
        aptos_account_1.AptosAccount.fromDerivePath("", mnemonic);
    }).toThrow(new Error("Invalid derivation path"));
});
test("accepts custom address", () => {
    const address = "0x777";
    const a1 = new aptos_account_1.AptosAccount(undefined, address);
    expect(a1.address().hex()).toBe(address);
});
...
test("Serializes/Deserializes", () => {
    const a1 = new aptos_account_1.AptosAccount();
    const a2 = aptos_account_1.AptosAccount.fromAptosAccountObject(a1.toPrivateKeyObject());
