...
test("submits bcs transaction", async () => {
    ...
    const scriptFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadScriptFunction(transaction_builder_1.TxnBuilderTypes.ScriptFunction.natural("0x1::coin", "transfer", [token], [
        transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account2.address())),
        transaction_builder_1.BCS.bcsSerializeUint64(717),
    ]));
...
test("submits multisig transaction", async () => {
    ...
    const token = new transaction_builder_1.TxnBuilderTypes.TypeTagStruct(transaction_builder_1.TxnBuilderTypes.StructTag.fromString("0x1::aptos_coin::AptosCoin"));
    const scriptFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadScriptFunction(transaction_builder_1.TxnBuilderTypes.ScriptFunction.natural("0x1::coin", "transfer", [token], [
        transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account4.address())),
        transaction_builder_1.BCS.bcsSerializeUint64(123),
    ]));
    const [{ sequence_number: sequnceNumber }, chainId] = await Promise.all([
...
test("submits bcs transaction simulation", async () => {
    ...
    const scriptFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadScriptFunction(transaction_builder_1.TxnBuilderTypes.ScriptFunction.natural("0x1::coin", "transfer", [token], [
        transaction_builder_1.BCS.bcsToBytes(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account2.address())),
        transaction_builder_1.BCS.bcsSerializeUint64(1000),
    ]));
    const [{ sequence_number: sequnceNumber }, chainId] = await Promise.all([
        client.getAccount(account1.address()),
        client.getChainId(),
...
test("submits multiagent transaction", async () => {
    ...
    const aliceSignature = new transaction_builder_1.TxnBuilderTypes.Ed25519Signature(alice
    ...
    const bobSignature = new transaction_builder_1.TxnBuilderTypes.Ed25519Signature(bob
        .signBuffer(transaction_builder_1.TransactionBuilder.getSigningMessage(multiAgentTxn))
        .toUint8Array());
