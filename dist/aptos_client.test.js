...
test("submits bcs transaction", async () => {
    ...
    const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
        (0, bcs_1.bcsToBytes)(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account2.address())),
        (0, bcs_1.bcsSerializeUint64)(717),
    ]));
...
test("submits transaction with remote ABI", async () => {
    ...
    accountResource = resources.find((r) => r.type === aptosCoin);
    expect(accountResource.data.coin.value).toBe("0");
    const builder = new transaction_builder_1.TransactionBuilderRemoteABI(client, {
...
test("submits multisig transaction", async () => {
    ...
    const token = new transaction_builder_1.TxnBuilderTypes.TypeTagStruct(transaction_builder_1.TxnBuilderTypes.StructTag.fromString("0x1::aptos_coin::AptosCoin"));
    const entryFunctionPayload = new transaction_builder_1.TxnBuilderTypes.TransactionPayloadEntryFunction(transaction_builder_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
        (0, bcs_1.bcsToBytes)(transaction_builder_1.TxnBuilderTypes.AccountAddress.fromHex(account4.address())),
        (0, bcs_1.bcsSerializeUint64)(123),
    ]));
    ...
    const txnBuilder = new transaction_builder_1.TransactionBuilderMultiEd25519((signingMessage) => {
        ...
        const bitmap = transaction_builder_1.TxnBuilderTypes.MultiEd25519Signature.createBitmap([
            0, 2,
        ]);
...
test("submits json transaction simulation", async () => {
    ...
    const transactionRes = (await client.simulateTransaction(account1.pubKey(), txnRequest, {
...
test("submits bcs transaction simulation", async () => {
    ...
    const rawTxn = await client.generateRawTransaction(account1.address(), entryFunctionPayload);
    const bcsTxn = aptos_client_1.AptosClient.generateBCSSimulation(account1.pubKey(), rawTxn);
