...
class AptosClient {
     ...
     */
    static generateBCSSimulation(accountPublicKey, rawTxn) {
    ...
    async generateTransaction(sender, payload, options) {
        ...
        try {
            // simulate txn
            const simulateResponse = await this.simulateTransaction(hex_string_1.HexString.ensure(options.publicKey), generatedPayload, {
     ...
     *
     */
    async simulateTransaction(accountPublicKey, rawTransaction, query) {
        const signedTxn = AptosClient.generateBCSSimulation(hex_string_1.HexString.ensure(accountPublicKey), rawTransaction);
