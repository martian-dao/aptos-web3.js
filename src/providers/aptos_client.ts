...
export class AptosClient {
  ...
  async getAccountTransactions(
    ...
    const { data } = await get<{}, Gen.Transaction[]>({
      url: this.nodeUrl,
      endpoint: `accounts/${HexString.ensure(
        accountAddress
      ).hex()}/transactions`,
  ...
  async getEventsByEventHandle(
    ...
    const { data } = await get<{}, Gen.Event[]>({
      ...
      endpoint: `accounts/${HexString.ensure(
        address
      ).hex()}/events/${eventHandleStruct}/${fieldName}`,
  ...
  async simulateTransaction(
    ...
    if (accountOrPubkey instanceof AptosAccount) {
      signedTxn = AptosClient.generateBCSSimulation(
        accountOrPubkey,
        rawTransaction
      );
  ...
  async submitBCSSimulation(
    ...
    const queryParams = {
      ...
      estimate_prioritized_gas_unit_price:
        query?.estimatePrioritizedGasUnitPrice ?? false,
    };
